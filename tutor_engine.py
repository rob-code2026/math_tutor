...
1. LATEX & MATH FORMATTING (STRICT RULES):
   - Always format fractions with \frac{numerator}{denominator}, write mixed numbers as 'whole \frac{num}{den}' with a space before \frac, and enclose all inline math in single dollar signs ... for proper LaTeX rendering
...
def sanitize_math_output(json_data):
  """Fixes Streamlit KaTeX and HTML rendering errors for mixed numbers, decimals, and fractions."""
  if not json_data:
    return json_data
  ...
  def clean_markdown_text(text):
    ...
    # 2. Correct common LLM typos (\fracrac, \ffrac -> \frac)
    text = re.sub(r'\\+fracrac', r'\\frac', text)
    text = re.sub(r'\\+ffrac', r'\\frac', text)

    # 3. Collapse double backslashes before 'frac' into a single backslash for raw LaTeX
    text = re.sub(r'\\+frac', r'\\frac', text)

    # 4. Ensure a space between any digit and \frac (e.g., "3\frac" -> "3 \frac")
  ...
  if isinstance(json_data, dict):
    ...
    # Apply HTML-specific formatting specifically to color_coded_html field
    if "workspace" in json_data and isinstance(json_data["workspace"], dict):
      if "color_coded_html" in json_data["workspace"]:
        json_data["workspace"]["color_coded_html"] = clean_html_text(
          json_data["workspace"]["color_coded_html"]
        )
